...
class GetInstagramProfile():
    ...
    def download_users_posts_with_periods(self, username):
        ...
        SINCE = datetime(2022, 10, 12)
        UNTIL = datetime(2022, 10, 14)

        for post in takewhile(lambda p: p.date > SINCE, dropwhile(lambda p: p.date > UNTIL, posts)):
            self.L.download_post(post, username)

    def download_hastag_posts(self, hashtag):
        for post in instaloader.Hashtag.from_name(self.L.context, self.L.context).get_posts():
            try:
                self.L.download_post(post, target='#' + hashtag)
            except:
                print("## ERROR AT : ", self.L.context, self.L.context)
